...
async def main():
    ...
    try:
        ...
        python_dir = Path(settings.STANDARDS_BASE_PATH) / 'python'
        if python_dir.exists():
            await importer.import_from_directory(python_dir, 'python', recursive=False)
        else:
            print(f"❌ Python standards directory not found: {python_dir}")

        # Could add other languages here
        # javascript_dir = Path(settings.STANDARDS_BASE_PATH) / 'javascript'
        # await importer.import_from_directory(javascript_dir, 'javascript')
